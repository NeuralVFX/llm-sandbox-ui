...
- Web-Interface:
  [llm-sandbox-ui](https://github.com/NeuralVFX/llm-sandbox-ui) \<— You
  are here
- Unreal Plugin:
  [unreal-llm-sandbox-plugin](https://github.com/NeuralVFX/unreal-llm-sandbox-plugin)

# What is this?

An Unreal Engine plugin that exposes a Python execution server,
enabling: - A web-based coding interface connected directly to Unreal -
...
- **LLM Execution** - Ask an LLM for help, with your code/errors in
  context
...
**[unreal-llm-sandbox-unreal](https://github.com/NeuralVFX/unreal-llm-sandbox-plugin)**
...
From the menu bar: `LLM Sandbox → Start Server` - The server runs at
`http://127.0.0.1:5002`
...
- Or create a new python file in your project’s `Content/Python/tools`
  directory \#### Tool Discovery:
- Tools registered in `Code Cells` are instantly availible to the LLM
- Tools added to `Content/Python/tools` are discovered on project
  restart \## To use:
- Open a `Prompt Cell`, Click the 🛠️ icon to activate Unreal tools, and
  write a prompt!

## View the tool Schema

Schemas are stored in a global variable `TOOL_SCHEMAS`, printing it
